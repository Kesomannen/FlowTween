...
internal class MaterialIcon : Label {
    string _iconName;
    ...
    public string IconName {
        ...
        set {
            _iconName = value;
            if (!TryGetCodepointByIconName(value, out var codepoint)) {
    ...
    static Dictionary<string, string> _iconNameToCodepoint;
    
    const string FallbackCodepoint = "eb8b";
    
    public new class UxmlFactory : UxmlFactory<MaterialIcon, UxmlTraits> { }

    public new class UxmlTraits : VisualElement.UxmlTraits {
        readonly UxmlStringAttributeDescription _icon = new() { name = "icon", defaultValue = "" };

        public override void Init(VisualElement ve, IUxmlAttributes bag, CreationContext cc) {
            base.Init(ve, bag, cc);
            var target = (MaterialIcon)ve;
            
            var icon = _icon.GetValueFromBag(bag, cc);
            target.AddToClassList("flow-material-icon");
            target.IconName = icon;
        }
    }

    public MaterialIcon() {
        AddToClassList("flow-material-icon");
    }

    static bool TryGetCodepointByIconName(string name, out string codepoint) {
        if (_iconNameToCodepoint == null) {
            _iconNameToCodepoint = new Dictionary<string, string>();
            var codepointLines = CodepointsString.Split("\n");
            
            foreach (var codepointLine in codepointLines) {
                var segments = codepointLine.Trim().Split(" ");
                if (segments.Length != 2) continue;
                _iconNameToCodepoint[segments[0]] = segments[1];
            }
        }

        return _iconNameToCodepoint.TryGetValue(name, out codepoint);
    }
    
    const string CodepointsString = @"
10k e951
10mp e952
11mp e953
123 eb8d
